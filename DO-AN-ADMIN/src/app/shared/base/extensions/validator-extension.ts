...
const PHONE_VN_REGEXP = /((01|02|03|04|05|06|07|08|09)+([0-9]{8,})\b)/;
...
export class ValidatorExtension {
  ...
  static minDate(min: Date, errorMessage?: string) {
    return (control: AbstractControl): ValidationErrors | null => {
      ...
      return value != null && value !== undefined && value < min ? {
        error: errorMessage || `Giá trị ngày tối thiểu là ${min.getDate()}/${min.getMonth() + 1}/${min.getFullYear()}`,
        min: { min, actual: control.value }
      } : null;
    };
  }
  ...
  static maxDate(max: Date, errorMessage?: string) {
    return (control: AbstractControl): ValidationErrors | null => {
      ...
      if (value.getFullYear() > max.getFullYear() ||
        ...
        return {
          error: errorMessage || `Giá trị ngày tối đa là ${max.getDate()}/${max.getMonth() + 1}/${max.getFullYear()}`,
          maxDate: true
        }
      }
      return null;
  ...
  static phoneNumberVN(errorMessage?: string) {
    return (control: AbstractControl): ValidationErrors | null => {
      ...
      return PHONE_VN_REGEXP.test(control.value) ? null : { error: errorMessage || 'Không đúng định dạng số điện thoại', phoneNumber: true };
    };
  }
  ...
  static codeText(errorMessage?: string) {
    return (control: AbstractControl): ValidationErrors | null => {
      ...
      return CODE_TEXT.test(control.value) ? null : { error: errorMessage || 'Không đúng định dạng mã', phoneNumber: true };
    };
  }
  ...
  static gtDateValidator(myForm: FormGroup, controlTarget: string, errorMessage: string) {
    return (control: AbstractControl) => {
      const controltaget: any = myForm.get(controlTarget);
      let control1: any = control.value;
      if (typeof control1 === 'string') {
      ...
      if (typeof control1 === 'number') {
        if (control1.toString().length === 8) {
          control1 = new Date(+control1.toString().substring(0, 4), +control1.toString().substring(4, 6) - 1, +control1.toString().substring(6, 8));
        }
        else if (control1.toString().length === 6) {
          control1 = new Date(+control1.toString().substring(0, 4), +control1.toString().substring(4, 6) - 1, 1);
        }
        else {
      ...
      if (typeof control2 === 'number') {
        if (control2.toString().length === 8) {
          control2 = new Date(+control2.toString().substring(0, 4), +control2.toString().substring(4, 6) - 1, +control2.toString().substring(6, 8));
        }
        else if (control2.toString().length === 6) {
          control2 = new Date(+control2.toString().substring(0, 4), +control2.toString().substring(4, 6) - 1, 1);
        }
        else {
  ...
  static gteDateMinValidator(myForm: FormGroup, controlTarget: string, errorMessage: string) {
    return (control: AbstractControl) => {
      const controltaget: any = myForm.get(controlTarget);
      let control1: any = control.value;
      if (typeof control1 === 'string') {
