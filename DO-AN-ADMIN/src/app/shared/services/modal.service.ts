...
import { ModalOptions } from '../model/modal';
...
export class ModalService {
  ...
  open(options: ModalOptions): Promise<any> {
    ...
    const size: any = options.size ? options.size : this.state.options.template || this.state.options.component ? 'xl' : 'lg';
  ...
  warn(options: ModalOptions): Promise<any> {
    ...
    options.okLabel = options.okLabel || 'Có';
    options.cancelLabel = options.cancelLabel || 'Đóng';
    this.state.options = options;
